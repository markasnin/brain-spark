...
function g3_perimeter(diff){
  ...
  if(type==='square'){
    ...
    body=`<polygon points="150,25 265,195 35,195" fill="${PAL.f[0]}88" stroke="${PAL.s[0]}" stroke-width="3"/>
      ${dimLine(35,205,265,205,a+' ס"מ',PAL.s[0])}
      ${dimLine(268,195,152,25,b+' ס"מ',PAL.s[0])}
      ${dimLine(148,25,32,195,c+' ס"מ',PAL.s[0])}`;
  }
...
function g5_lShape(diff){
  ...
  const Wp=W*sc,Hp=H*sc,wp=w*sc,hp=h*sc,ox=(300-Wp)/2,oy=(225-Hp)/2;
  return {type:'shapes',cat:'shapes',diff,
    text:`מה שטח צורת L? (${W}×${H} פחות ${w}×${h})`,answer,
    pts:ptsForQ('shapes',diff),
    shapeHtml:svgWrap(300,225,`
      ...
      ${dimLine(ox,oy-16,ox+Wp,oy-16,W+'',PAL.s[3])}
      ${dimLine(ox-18,oy,ox-18,oy+Hp,H+'',PAL.s[3])}
...
function g6_scaleFactor(diff){
  ...
  const pw=Math.min(nW*9,200),ph=Math.min(nH*9,130);
  const ox=(300-pw)/2,oy=(215-ph)/2;
  ...
  return {type:'shapes',cat:'shapes',diff,
    ...
    pts:ptsForQ('shapes',diff),
    shapeHtml:svgWrap(300,215,`
      <rect x="${ox}" y="${oy}" width="${pw}" height="${ph}" fill="${PAL.f[1]}77" stroke="${PAL.s[1]}" stroke-width="3" rx="5"/>
      <rect x="${ox-spw-10}" y="${oy}" width="${spw}" height="${sph}" fill="${PAL.f[3]}77" stroke="${PAL.s[3]}" stroke-width="2" rx="3"/>
      <line x1="${ox-10}" y1="${oy+sph/2}" x2="${ox-5}" y2="${oy+ph/2}" stroke="#ffd32a" stroke-width="1.5" stroke-dasharray="4,3"/>
      ${tx(ox-spw/2-10,oy+sph+12,oW+'×'+oH,'#ffe07e',10)}
      ${dimLine(ox,oy-16,ox+pw,oy-16,nW+'',PAL.s[1])}
      ${dimLine(ox-18,oy,ox-18,oy+ph,nH+'',PAL.s[1])}
      ${tx(ox+pw/2,oy+ph/2+7,'?','#ffffffcc',20)}
      ${tx(ox+pw/2,oy+ph+18,'×'+sc+' מכל מימד','#ffd32a',11)}`),
